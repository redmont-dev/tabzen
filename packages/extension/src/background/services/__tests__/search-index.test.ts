...
import { registerSearchIndex, type TabSearchResult } from '../search-index';
...
describe('SearchIndex', () => {
  ...
  async function registerAndDispatch(query: string, scope: 'tabs' | 'sessions' | 'all' = 'tabs') {
    registerSearchIndex(bus);
    const response = await bus.dispatch({ action: 'searchTabs', query, scope });
    return { ok: response.ok, data: (response.data ?? []) as TabSearchResult[] };
  }
